...
export const TempSessionService = {
  save(user) {
    ...
    if (user.admin_id != null) localStorage.setItem('hindPharmaAdminId', String(user.admin_id)); else localStorage.removeItem('hindPharmaAdminId');
    if (user.business_name) localStorage.setItem('hindPharmaBusinessName', user.business_name); else localStorage.removeItem('hindPharmaBusinessName');
    if (user.subscription_expiry) localStorage.setItem('hindPharmaSubscriptionExpiry', user.subscription_expiry); else localStorage.removeItem('hindPharmaSubscriptionExpiry');
  ...
  get() {
    ...
    if (!token || !username || !VALID_ROLES.includes(role) || version!==SESSION_VERSION || loginTime<=0 || Date.now()-loginTime>=SESSION_DURATION_MS || (!['super_admin'].includes(role)&&expiry&&new Date(`${expiry}T23:59:59`)<new Date())) {
      this.clear(); return null;
    }
    return {token,username,role,admin_id:Number(localStorage.getItem('hindPharmaAdminId')||0)||null,business_name:localStorage.getItem('hindPharmaBusinessName')||'',subscription_expiry:expiry||null,loginTime};
  },
  isLoggedIn(){return Boolean(this.get())},
  hasRole(...roles){const s=this.get();return Boolean(s&&roles.includes(s.role))},
