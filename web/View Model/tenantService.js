...
function slugFromLocation(){const match=location.pathname.match(/\/hindhealthcare\/pharmaflow\/([^/]+)/i);if(match)return match[1].toLowerCase();const legacy=location.pathname.match(/\/shop\/([^/]+)/i);if(legacy)return legacy[1].toLowerCase();const query=new URLSearchParams(location.search).get('shop');return query?.toLowerCase()||localStorage.getItem(CURRENT_SHOP_KEY)||'hind-pharma'}
