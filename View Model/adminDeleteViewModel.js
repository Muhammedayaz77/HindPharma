import { TempDataService } from '../temp/temp_file_dataService.js?v=20260830-7';
...
const confirmDelete=(type,name)=>window.confirm(`Delete this ${type}?\n\n${name}\n\nThis entry will be removed from the website.`);
...
$('medicalTable').addEventListener('click',async event=>{const button=event.target.closest('[data-medical]');if(!button)return;const item=JSON.parse(decodeURIComponent(button.dataset.medical));if(!confirmDelete('medical',item.name))return;await service.deleteMedical(item);msg('medicalMessage','Medical deleted successfully.',true);await load();});
$('productTable').addEventListener('click',async event=>{const button=event.target.closest('[data-product]');if(!button)return;const item=JSON.parse(decodeURIComponent(button.dataset.product));if(!confirmDelete('product',item.name||item.id))return;await service.deleteProduct(item);msg('productMessage','Product deleted successfully.',true);await load();});
$('userTable').addEventListener('click',async event=>{const button=event.target.closest('[data-user]');if(!button)return;const item=JSON.parse(decodeURIComponent(button.dataset.user));if(!confirmDelete('user',item.username))return;await service.deleteUser(item);msg('userMessage','User deleted successfully.',true);await load();});
