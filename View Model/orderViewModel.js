...
function render(){list.innerHTML=order.length?order.map((item,index)=>`<div class="item"><span>${index+1}. ${esc(item.name)}</span><span class="qty"><button type="button" data-action="minus" data-index="${index}">−</button>${item.quantity}<button type="button" data-action="plus" data-index="${index}">+</button><button type="button" class="remove" data-action="remove" data-index="${index}">×</button></span></div>`).join(''):'<p>Your order is empty.</p>';document.getElementById('next').disabled=!order.length}
