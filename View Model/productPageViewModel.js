...
document.getElementById('minus').onclick=()=>qty.value=Math.max(1,(+qty.value||1)-1);document.getElementById('plus').onclick=()=>qty.value=(+qty.value||1)+1;document.getElementById('cancel').onclick=()=>modal.classList.remove('show');document.getElementById('add').onclick=()=>{const quantity=Math.max(1,parseInt(qty.value,10)||1);if(!selected)return;const key=`product:${selected.id||selected.name}`,old=order.find(item=>item.key===key);if(old)old.quantity+=quantity;else order.push({key,productId:selected.id,name:selected.name,quantity});localStorage.setItem('hindPharmaOrder',JSON.stringify(order));updateCart();modal.classList.remove('show')};document.getElementById('cartBtn').onclick=()=>location.href='order.html';search.addEventListener('input',event=>searchProducts(event.target.value));document.getElementById('intro').textContent=`Ordering for ${medical}. Tap anywhere on a product card to select it.`;updateCart();loadProducts();TempSessionService.startExpiryWatcher(()=>location.replace('login.html'));
