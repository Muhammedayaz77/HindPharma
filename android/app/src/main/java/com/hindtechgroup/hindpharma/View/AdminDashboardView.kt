...
@Composable
fun AdminDashboardView(user: SessionUser, onBack: () -> Unit, viewModel: AdminDashboardViewModel = viewModel()) {
    ...
    Column(Modifier.fillMaxSize().padding(20.dp)) {
        ...
        Text("Business Control Center", style = MaterialTheme.typography.titleMedium)
        StatCard("ACTIVE USERS", viewModel.userCount)
...
private fun StatCard(label: String, value: Int) {
    Card(Modifier.fillMaxWidth().padding(vertical = 5.dp)) {
        Column(Modifier.padding(16.dp)) { Text(label); Text(value.toString(), style = MaterialTheme.typography.headlineMedium) }
    }
}
