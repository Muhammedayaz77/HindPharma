...
struct HomeView: View {
    ...
    var body: some View {
        Group {
            if let user = viewModel.sessionUser { ShopHomeView(user: user, viewModel: viewModel) }
            else { LoginView(viewModel: viewModel) }
        }
    }
}
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 16) {
                ...
                Button { Task { await viewModel.login() } } label: { if viewModel.isLoading { ProgressView() } else { Text("LOGIN") } }
                    .frame(maxWidth: .infinity).buttonStyle(.borderedProminent).disabled(viewModel.isLoading)
            ...
            List {
                ...
                Section("Home") {
                    ForEach(actions.filter { $0.1.contains(user.role) }, id: \.0) { action in
                        switch action.0 {
                        case "Daily Calling": NavigationLink(action.0) { DailyCallingView(user: user) }
                        case "Medical List", "Products", "Order": NavigationLink(action.0) { NativeOrderFlowView(user: user) }
                        case "Manager Dashboard": NavigationLink(action.0) { ManagerDashboardView(user: user) }
                        case "Admin Dashboard": NavigationLink(action.0) { AdminDashboardView(user: user) }
                        default: NavigationLink(action.0) { SuperAdminDashboardView(user: user) }
                        }
                    }
                }
...
private struct NativeOrderFlowView: View {
    ...
    var body: some View {
        Group {
            if stage == 0 || medical == nil { MedicalListView(user: user) { m in medical = m; stage = 1 } }
            else if stage == 1 { ProductListView(user: user, medical: medical!, onOrder: { items in cart = items; stage = 2 }) }
            else { OrderView(user: user, medical: medical!, initialItems: cart, onBack: { stage = 1 }, onNewOrder: { cart = []; medical = nil; stage = 0 }) }
        }
    }
}
